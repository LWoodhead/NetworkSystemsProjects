...
void echo(int connfd) 
{
    ...
    if((strncmp(request_uri,"/",1) && strlen(request_uri) == 1) || (strncmp(request_uri,"index.html",10) && strlen(request_uri) == 1)){
        // printf("matched the index.html\n");
        bzero(corrected_path,100);
        strcpy(corrected_path,"index.html");
    }else{
        // \printf("did not match the index.html\n");
        //Parse uri to get the correct file_path
        bzero(file_path,100);
        memcpy(file_path,request_uri,strlen(request_uri));
        ...
        bzero(file_type,4);
        memcpy(file_type,&file_path[last_period_index+1],strlen(file_path) - last_period_index);
        printf("the file type is %s\n",file_type);
        bzero(content_type,20);
        if(strncmp(file_type,"html",4)){
            strcpy(content_type,"text/html");
        }else if(strncmp(file_type,"txt",3)){
            strcpy(content_type,"text/plain");
        }else if(strncmp(file_type,"png",3)){
            strcpy(content_type,"image/png");
        }else if(strncmp(file_type,"gif",3)){
            strcpy(content_type,"image/gif");
        }else if(strncmp(file_type,"jpg",3)){
            strcpy(content_type,"image/jpg");
        }else if(strncmp(file_type,"css",3)){
            strcpy(content_type,"test/css");
        }else if(strncmp(file_type,"js",2)){
            strcpy(content_type,"application/javascript");
        }
        bzero(corrected_path,100);
        memcpy(corrected_path,&file_path[1],strlen(file_path)-1);
        printf("corrected path is %s\n",corrected_path);
    }
    
    file_ptr = fopen(corrected_path,"r");
    ...
    printf("after thread file open and close\n");
    ...
    if(valid_file == 1){
        ...
        snprintf(buf,MAXBUF,"%s 500 Internal Server Error\r\n",request_version);
    }
...
void echo(int connfd) 
{
    ...
    if(file_ptr == NULL){
        ...
        //Seek back to the beginning and write the file into its buffer
        fseek(file_ptr,0,SEEK_SET);
        //Set aside 200 bytes for our http header
        if(file_length + 200 <= MAXBUF){
            ...
            bzero(buf,BUFSIZ);
            snprintf(buf,MAXBUF,"%s%s",header_buf,file_buf);
            write(connfd, buf,header_size + file_length);
        }else{
            //Otherwise first read the first MAXBUF-200 bytes into buf and send
            bzero(header_buf,200);
            snprintf(header_buf,200,"%s 200 Document Follows\r\nContent-Type: %s\r\nContent-Length: %zu\r\n\r\n",request_version,content_type,file_length);
            int header_size = strlen(header_buf);
            bzero(file_buf,MAXBUF);
